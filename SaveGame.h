...
struct statusGame
{
	...
	int bullet_status[5]; // trang thai cua tung vien dan
	float bullet_x[5];
	float bullet_y[5];
...
class SaveGame
{
	statusGame status;
public:
	void saveGameOutFile(int score, string name, float vx, float vy, float bx, float by, Paddle paddle1, vector <Brick*> shape, int level, int life, GunMode gun);
	
	void readSaveGame(float & vx, float &vy, Ball &ball, Paddle &paddle1, Grid & grid, int & life, GunMode & gun);
